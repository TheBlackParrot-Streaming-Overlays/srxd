...
function postToSRXDEventChannel(event, data) {
	let message = {
		event: event
	};
	if(data) {
	...
	console.log(message);
	srxdEventChannel.postMessage(message);
}

srxdEventChannel.onmessage = function(message) {
	//console.log(message);
	data = message.data;
...
broadcastFuncs = {
	...
	settingsOverlayLoaded: function(message) {
		postToSettingsEventChannel("SRXDOverlayExists", {version: overlayRevision});
	},

	settingsKeysSRXD: function(message) {
